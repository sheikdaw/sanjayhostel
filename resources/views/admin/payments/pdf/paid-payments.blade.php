<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
    <title>{{ $title }}</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: 'DejaVu Sans', sans-serif; font-size: 10px; color: #333; padding: 15px; }
        .header { text-align: center; border-bottom: 3px solid #2e7d32; padding-bottom: 12px; margin-bottom: 15px; }
        .header h1 { font-size: 18px; color: #2e7d32; }
        .header p { font-size: 10px; color: #666; margin-top: 3px; }
        .report-info { display: flex; justify-content: space-between; margin-bottom: 12px; font-size: 9px; background: #e8f5e9; padding: 6px 10px; border-radius: 4px; flex-wrap: wrap; }
        .report-info .label { font-weight: bold; }
        table { width: 100%; border-collapse: collapse; font-size: 8.5px; }
        table th { background: #2e7d32; color: white; padding: 5px 3px; text-align: left; font-size: 7.5px; text-transform: uppercase; letter-spacing: 0.3px; }
        table td { padding: 3px; border-bottom: 1px solid #e0e0e0; }
        table tr:nth-child(even) { background: #f8f9fa; }
        .badge { padding: 1px 6px; border-radius: 8px; font-size: 7px; font-weight: bold; display: inline-block; background: #c8e6c9; color: #2e7d32; }
        .footer { margin-top: 15px; padding-top: 8px; border-top: 1px solid #e0e0e0; text-align: center; font-size: 8px; color: #999; }
        .summary-box { margin-top: 12px; padding: 8px 12px; background: #e8f5e9; border-radius: 4px; border-left: 4px solid #2e7d32; }
        .summary-box h3 { font-size: 11px; color: #2e7d32; margin-bottom: 6px; }
        .summary-grid { display: flex; flex-wrap: wrap; gap: 10px; }
        .summary-item { font-size: 9px; }
        .summary-item .number { font-weight: bold; font-size: 11px; }
        .summary-item .label { color: #666; }
        .text-success { color: #2e7d32; }
        .text-primary { color: #1a237e; }
    </style>
</head>
<body>
    <div class="header">
        <h1>✅ {{ $title }}</h1>
        <p>Generated on: {{ $generated_at }}</p>
    </div>

    <div class="report-info">
        <div><span class="label">Total Paid Records:</span> {{ $summary['total'] }}</div>
        <div><span class="label">Total Rent:</span> ₹{{ number_format($summary['total_rent'], 2) }}</div>
        <div><span class="label">Total Collected:</span> ₹{{ number_format($summary['total_collected'], 2) }}</div>
        <div><span class="label">Generated By:</span> {{ $user->name }}</div>
    </div>

    @if(count($payments) > 0)
        <table>
            <thead>
                <tr>
                    <th>Receipt No</th>
                    <th>Resident</th>
                    <th>Hostel</th>
                    <th>Room</th>
                    <th>Month</th>
                    <th>Year</th>
                    <th>Rent (₹)</th>
                    <th>Disc (₹)</th>
                    <th>Fine (₹)</th>
                    <th>Cash (₹)</th>
                    <th>UPI (₹)</th>
                    <th>Paid (₹)</th>
                    <th>Date</th>
                    <th>Txn ID</th>
                </tr>
            </thead>
            <tbody>
                @foreach($payments as $payment)
                    @php
                        $monthName = date('F', mktime(0, 0, 0, $payment->month, 1));
                        $totalPaid = $payment->cash_paid_amount + $payment->upi_paid_amount;
                    @endphp
                    <tr>
                        <td>{{ $payment->receipt_no }}</td>
                        <td>{{ $payment->resident->name ?? 'N/A' }}</td>
                        <td>{{ $payment->resident->hostel->hostel_name ?? 'N/A' }}</td>
                        <td>#{{ $payment->resident->room->room_no ?? 'N/A' }}</td>
                        <td>{{ $monthName }}</td>
                        <td>{{ $payment->year }}</td>
                        <td>{{ number_format($payment->rent_amount, 2) }}</td>
                        <td>{{ number_format($payment->discount_amount, 2) }}</td>
                        <td>{{ number_format($payment->fine_amount, 2) }}</td>
                        <td>{{ number_format($payment->cash_paid_amount, 2) }}</td>
                        <td>{{ number_format($payment->upi_paid_amount, 2) }}</td>
                        <td><strong>{{ number_format($totalPaid, 2) }}</strong></td>
                        <td>{{ $payment->payment_date }}</td>
                        <td>{{ $payment->transaction_id ?? '' }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>

        <div class="summary-box">
            <h3>📊 Summary</h3>
            <div class="summary-grid">
                <div class="summary-item">
                    <div class="number text-success">{{ $summary['total'] }}</div>
                    <div class="label">Total Paid</div>
                </div>
                <div class="summary-item">
                    <div class="number text-primary">₹{{ number_format($summary['total_rent'], 2) }}</div>
                    <div class="label">Total Rent</div>
                </div>
                <div class="summary-item">
                    <div class="number text-success">₹{{ number_format($summary['total_collected'], 2) }}</div>
                    <div class="label">Total Collected</div>
                </div>
            </div>
        </div>
    @else
        <div style="text-align:center; padding:30px; background:#f5f5f5; border-radius:8px;">
            <h2 style="color:#757575;">No Paid Payments Found</h2>
            <p style="color:#666; font-size:11px;">No paid payment records match the selected filters</p>
        </div>
    @endif

    <div class="footer">
        Generated by: {{ $user->name }} ({{ $user->role }}) | {{ $generated_at }}
    </div>
</body>
</html>
